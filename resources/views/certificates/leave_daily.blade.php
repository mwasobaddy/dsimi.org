...
<div class="header-container">
    ...
    <div class="title">
        CERTIFICAT DE {{ $leave->leave_type_name }}
    </div>

    <div class="content">
        Je soussigné, Docteur MEITE Djoussoufou, Directeur des Soins Infirmiers, Maternels
        et Infantiles (DSIMI) certifie que {{ $leave->employee_title }} 
        {{ $leave->employee_name }}, matricule {{ $leave->employee_id_number }}, 
        {{ $leave->employee_position }} à {{ $leave->employee_department }}, bénéficiaire 
        d'une {{ strtolower($leave->leave_type_name) }} de {{ $leave->total_leave_days }} jours 
        au titre de l'année {{ $leave->year }}, cesse service le 
        {{ \Carbon\Carbon::parse($leave->start_date)->format('d/m/Y') }}.
    </div>

    <div class="leave-details">
        <strong>Type de congé:</strong> {{ $leave->leave_type_name }}<br>
        <strong>Motif:</strong> {{ $leave->leave_reason ?? 'Non spécifié' }}<br>
        <strong>Durée:</strong> {{ $leave->total_leave_days }} jours
    </div>

    <div class="content">
        L'intéressé(e) reprendra service le 
        {{ \Carbon\Carbon::parse($leave->end_date)->addDays(1)->format('d/m/Y') }}, 
