    ...
    public function index()
    {
        ...
        if (Auth::check()) {
            ...
            if ($user->type =='employee') {
                ...
                $present_employees    = AttendanceEmployee::where('date', '=', $currentDate)->get()->pluck('employee_id');

                $notClockIns = Employee::where('supervisor_n1', '=', $emp->name)
                ->orWhere('supervisor_n2', '=', $emp->name)
                ->whereIn('user_id', $present_employees)->get();
            ...
            else if ($user->type  == 'Administrator (Employee)') {
                $emp = Employee::where('user_id', '=', $user->id)->first();
                ...
                return view('dashboard.dashboard', compact('arrEvents', 'announcements', 'employees', 'activeJob', 'inActiveJOb', 'meetings', 'countEmployee', 'countUser', 'countTicket', 'countOpenTicket', 'countCloseTicket', 'notClockIns', 'accountBalance', 'totalPayee', 'totalPayer', 'users', 'plan', 'storage_limit', 'officeTime',  'employeeAttendance'));
