...
{
    protected $leaveCertificateService;

    public function __construct(LeaveCertificateService $leaveCertificateService)
    {
        $this->leaveCertificateService = $leaveCertificateService;
    }
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('Create Leave')) {
            ...
            if ($leave_type->days >= $total_leave_days) {
                ...
                // Get the employee details for the certificate
                $employee = Employee::with('designation', 'department')->find($request->employee_id);
                
                // Add employee details to leave object for certificate generation
                $leave->employee = [
                    'title' => $employee->gender == 'M' ? 'M.' : 'Mme',
                    'full_name' => $employee->name,
                    'matricule' => $employee->employee_id,
                    'position' => $employee->designation->name ?? $employee->position,
                    'department' => $employee->department->name ?? '',
                    'leave_type' => $leave_type->title
                ];
                
                // Add additional certificate fields
                $leave->days = $total_leave_days;
                $leave->year = date('Y');
                ...
                try {
                    $certificateFilename = $this->leaveCertificateService->generateCertificate($leave);
                    
                    // Save the certificate path to the leave record
                    $leave->certificate_path = $certificateFilename;
                    $leave->save();
                } catch (\Exception $e) {
                    // Log the error but don't stop the leave creation process
                    \Log::error('Failed to generate leave certificate: ' . $e->getMessage());
                }
