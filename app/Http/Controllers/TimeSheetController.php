    ...
    public function index(Request $request)
    {
        if(\Auth::user()->can('Manage TimeSheet'))
        {
            ...
            if(\Auth::user()->type == 'employee')
            {
                ...
                // Get employees under this line manager with their IDs
                $employeesList = Employee::where(function ($query) {
                    $query->where('supervisor_n1', \Auth::user()->name)
                          ->orWhere('supervisor_n2', \Auth::user()->name);
                })->pluck('name', 'user_id'); // Changed to use 'id' instead of 'user_id' for the dropdown
            
                // Get the employee IDs for filtering timesheets
                $employeeIds = Employee::where(function ($query) {
                    $query->where('supervisor_n1', \Auth::user()->name)
                          ->orWhere('supervisor_n2', \Auth::user()->name);
                })->pluck('user_id')->toArray();
            
                // Start building the timesheet query
                ...
                // Get the results with employee relationship
                $timeSheets = $timesheets->with('employee')->get();
                ...
                //     'request_employee' => $request->employee
                // ]);
            }
            else
            {
