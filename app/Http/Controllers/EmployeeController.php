...
class EmployeeController extends Controller
{
    ...
    public function import(Request $request)
    {
        ...
        $totalEmployees = count($employees) - 1;
        $errorArray = [];
        ...
        for ($i = 1; $i <= $totalEmployees; $i++) {
            ...
            if (!empty($employeeByEmail) && !empty($userByEmail)) {
                ...
                $user = new User();
                $user->name = $employee[0] . ' ' . $employee[1];
            ...
            $employeeData->first_name = $employee[0];
            $employeeData->last_name = $employee[1];
            $employeeData->name = $employee[0] . ' ' . $employee[1];
            $employeeData->dob = $employee[2];
            $employeeData->gender = $employee[3];
            $employeeData->contract_type = $employee[4];
            $employeeData->email = $employee[5];
            $employeeData->services = !empty($employee[6]) ? $employee[6] : '-';
            try {
                $employeeData->civil_service_doe = \Carbon\Carbon::createFromFormat('d F Y', $employee[7])->format('Y-m-d');
            } catch (\Exception $e) {
                $employeeData->civil_service_doe = '';
            }

            $employeeData->account_holder_name = !empty($employee[8]) ? $employee[8] : '';
            $employeeData->account_number = !empty($employee[9]) ? $employee[9] : '';
            $employeeData->bank_name = !empty($employee[10]) ? $employee[10] : '';
            $employeeData->bank_identifier_code = !empty($employee[11]) ? $employee[11] : '';
            $employeeData->branch_location = !empty($employee[12]) ? $employee[12] : '';

            $branch = Branch::where('name', $employee[13])->first();
            $employeeData->branch_id = $branch ? $branch->id : 0;

            $department = Department::where('name', $employee[14])->first();
            $employeeData->department_id = $department ? $department->id : 0;

            $designation = Designation::where('name', $employee[15])->first();
            $employeeData->designation_id = $designation ? $designation->id : 0;

            $employeeData->salary_type = !empty($employee[16]) ? $employee[16] : '-';
            $employeeData->salary = !empty($employee[17]) ? $employee[17] : 0;
        ...
        if (empty($errorArray)) {
            ...
            $data['status'] = 'error';
            $data['msg'] = count($errorArray) . ' ' . __('Record(s) failed to import out of ' . $totalEmployees . ' record(s)');
